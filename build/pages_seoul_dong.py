# -*- coding: utf-8 -*-
"""서울 25개 자치구 행정동(行政洞) 페이지 — 1·2·3동 통합형, 244개.

각 페이지 ~2,200자 한글 콘텐츠 + 동(洞) 고유 데이터 + JSON-LD.
...
def _district_lookup():
    seoul = CITIES["seoul"]
    return {d["slug"]: d for d in seoul["districts"]}

DIST = _district_lookup()
...
def _adjacent_dongs(district_slug, current_slug, limit=4):
    """같은 구의 다른 행정동 — 인근 거점으로 4개 추출."""
    dongs = DONGS_BY_DISTRICT[district_slug]
...
def _faqs(name, district_name, near, zone, eta):
    return [
        ...
        (f"{name}에서도 24시 콜이 되나요?", f"네. {name}은(는) {ZONE_OVERVIEW[zone].split(',')[0]}{'으' if zone.endswith(('IT','관광')) else ''}로, 24시간 매니저 배차가 가능합니다."),
...
def dong_page(dong):
    ...
    adjacent = _adjacent_dongs(district_slug, slug, 4)
    adj_links = " · ".join(f'<a href="/locations/seoul/{district_slug}/dong/{a[1]}/" style="color:var(--rose)">{a[0]}</a>' for a in adjacent)
    adj_names = ", ".join(a[0] for a in adjacent)

    # ---- 콘텐츠 ----
    hero = f"""<section class="hero"><div class="hero-inner">
<div class="hero-copy reveal">
<span class="eyebrow"><span class="pulse"></span>서울 {district_name} · {name}</span>
<h1>{name}<br><span class="grad">평균 {eta}분</span><br><span class="serif">도착 약속.</span></h1>
<p class="lead">{char_line}. 마사지꾼은 {name}을(를) 포함한 {district_name} {len(DONGS_BY_DISTRICT[district_slug])}개 행정동을 24시간 단일 디스패치로 운영합니다. 평균 도착 {eta}분, 자문 트레이너 가이드라인, 156명 활동 매니저 — 전화 한 통이면 {near_first} 인근까지 매니저가 출발합니다.</p>
<div class="actions">
<a class="btn btn-primary" href="tel:{SITE['phone_raw']}">📞 {SITE['phone_display']} 예약 →</a>
<a class="btn btn-ghost" href="/locations/seoul/{district_slug}/">{district_name} 전체 보기</a>
</div>
<div class="trust">
...
<div class="floating fl-2">권역 데이터 · 2026</div>
    ...
    breadcrumbs = breadcrumb_html([
        ("홈","/"),("지역","/locations/"),("서울특별시","/locations/seoul/"),
        (district_name,f"/locations/seoul/{district_slug}/"),
        (name,f"/locations/seoul/{district_slug}/dong/{slug}/"),
    ])
    ...
    peak_block = note_card(3, f"{name} 콜 분포 — 시간대별 패턴", [
        f"{name}은(는) {ZONE_PEAK[zone]}. 야간(23~05시) 콜 비중은 {night}%로, 서울 전 권역 평균과 비교했을 때 {'두드러지게 높은' if night >= 40 else ('대체로 평균적인' if night >= 32 else '비교적 차분한')} 수준입니다.",
    ...
    adj_block = note_card(5, f"{district_name}의 인접 행정동", [
        f"{name} 외에도 {district_name}에는 {len(DONGS_BY_DISTRICT[district_slug])}개의 행정동이 운영됩니다. 인접 동까지 매니저 배차 동선이 공유되므로, {name} 가용 매니저가 적은 시점에는 인접 동에서 가장 가까운 매니저가 배정됩니다.",
        f"인근 거점: {adj_names} 등. 각 동의 상세 권역 정보는 다음 페이지에서 확인하실 수 있습니다: {adj_links}.",
        f"{district_name} 전체 운영 개요는 <a href=\"/locations/seoul/{district_slug}/\" style=\"color:var(--rose)\">{district_name} 페이지</a>에서 한눈에 볼 수 있습니다.",
    ])
    ...
    rec_s = next(s for s in SERVICES if s["slug"] == rec_service_slug)
    second_s = next(s for s in SERVICES if s["slug"] == ("aroma" if rec_service_slug != "aroma" else "swedish"))
...
<div class="meta"><b>{r['name']}</b><span>·</span><span>서울 {district_name} {name}</span><span>·</span><span>{r['course']}</span></div>
</div>""" for r in revs)
...
{section_head(f"SEOUL · {district_name}", f"{name} 권역 운영 노트", "5개월 실 배차 데이터로 정리한 동(洞) 단위 1차 데이터.")}
...
<div class="signoff">📝 책임 편집: 본사 {district_name} 운영팀 · 자문 검수: 박지연·정민호·윤하늬 · 데이터: 2025-11~2026-03 실 배차 로그 · 최종 갱신: 2026-05-28</div>
    ...
    desc = f"서울 {district_name} {name} 출장마사지. {char_line}. 평균 도착 {eta}분, 야간 콜 {night}%, {rec_course} 인기. 24시 응답 {SITE['phone_display']}."
    ...
    local_ld = {
        ...
        "url":f"{SITE['base_url']}/locations/seoul/{district_slug}/dong/{slug}/",
        "telephone":SITE["phone_tel"],"priceRange":"₩₩",
        "address":{"@type":"PostalAddress","addressLocality":name,"addressRegion":f"서울특별시 {district_name}","addressCountry":"KR"},
    ...
    return page(title, desc, f"/locations/seoul/{district_slug}/dong/{slug}/", body, ld_objs=[
        organization_ld(),
        breadcrumb_ld([
            ("홈","/"),("지역","/locations/"),("서울특별시","/locations/seoul/"),
            (district_name,f"/locations/seoul/{district_slug}/"),
...
def all_seoul_dong_pages():
    for dist_slug, dongs in DONGS_BY_DISTRICT.items():
        for name, slug, zone, near, char_line in dongs:
            ...
            yield (f"/locations/seoul/{dist_slug}/dong/{slug}/index.html", dong_page(dong))
