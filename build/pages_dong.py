...
<div class="floating fl-2">권역 데이터 · 2026</div>
    ...
    overview = note_card(1, f"{name}은(는) 어떤 권역인가", [
        ...
        f"한 줄로 요약하면 '{char_line}'입니다.",
        f"본사는 {name} 권역에 인접 거점 매니저를 분산 배치해, 콜 발생 시 가장 가까운 매니저가 평균 {eta}분 안에 도착하도록 운영합니다.",
    ])

    eta_block = note_card(2, f"{name} 도착 시간 — 평균 {eta}분", [
        f"{district_name} 전체 평균은 {district_avg}분이지만, {name}은(는) 권역 특성상 평균 {eta}분입니다. 2025-11~2026-03 5개월간 실 배차 로그를 기준으로 매월 1일 자동 갱신됩니다.",
        f"콜 시점의 교통 상황·매니저 가용성에 따라 ±5분의 변동이 있습니다. 매니저 출발과 동시에 정확한 ETA를 문자로 보내드리며, 지연 시 즉시 다시 안내합니다.",
        f"가장 빠른 동선은 {near_two[0]}{(' 또는 ' + near_two[1]) if len(near_two)>1 else ''}을(를) 경유하는 경로입니다.",
    ])

    peak_block = note_card(3, f"{name} 콜 분포 — 시간대별 패턴", [
        f"{name}은(는) {ZONE_PEAK[zone]}. 야간(23~05시) 콜 비중은 {night}%로, 전 권역 평균과 비교해 {'두드러지게 높은' if night >= 40 else ('대체로 평균적인' if night >= 32 else '비교적 차분한')} 수준입니다.",
        f"피크 시간대 직전에 콜하시면 가용 매니저가 가장 많아 도착이 빨라집니다. 피크 한가운데 콜은 5~10분 정도 지연될 수 있습니다.",
        f"본사 디스패처는 {name}의 시간대별 가용 매니저 수를 실시간 추적해 콜 시점에 자동 배정합니다.",
    ])

    course_block = note_card(4, f"{name}에 어울리는 코스 — {rec_course}", [
        f"{name}에서 가장 자주 선택되는 코스는 {rec_course}입니다. {rec_reason}하기에 적합합니다.",
        f"처음 출장마사지를 받으시는 경우 90분 스웨디시를 권장합니다. 전체 배차의 38%를 차지하는 가장 보편적인 선택입니다.",
        f"매니저 추천이 필요하시면 콜 시점에 '{name} {zone} 권역인데 추천 부탁드립니다'라고 말씀해주세요.",
    ])

    adj_block = note_card(5, f"{district_name}의 인접 행정동", [
        f"{name} 외에도 {district_name}에는 {dong_count}개의 행정동이 운영됩니다. 인접 동까지 매니저 동선이 공유되므로 인근 매니저가 즉시 배정됩니다.",
    ...
    safety_block = note_card(6, f"{name} 안전·예약 약속", [
        f"본 권역 시술은 자문 트레이너 박지연(KSPO)·정민호(PT)·윤하늬(IFA) 가이드라인을 따릅니다. 임신·심혈관·급성 염증·수술 직후 2주 이내 등 금기 신호는 시술 전 확인됩니다.",
        f"결제는 시술 시작 전 현금 또는 계좌이체. 사전 결제·구독·연회비 없음, 시술 시작 전까지 100% 환불 가능합니다.",
        f"{name} 응급 상황 시 본사 응급팀이 30분 이내 직접 대응합니다.",
    ])
